...
kable(propWins, caption = "Proportion of successful navigations, using previously generated trees")
```
...
kable(propNewWins, caption = "Proportion of successful navigations, using newly generated trees")
```
...
Can see that when measuring performance against the same trees, the model performs very well with a proportion of 0.99 passing. However, when using new trees, the model performs extremely poorly with no drones passing. This means that the model has not learned how to avoid objects, but simply how to avoid the given set of objects. 
...
kable(propWinsAdj, caption = "Proportion of successful navigations, using previously generated trees")
winNewObjs_adj = c()
...
kable(propNewWinsAdj, caption = "Proportion of successful navigations, using newly generated trees")
```
### ii

We can see that in sample win probability has decreased from 0.99 to 0.81, a 18% decrease. However, we see an increase from 0 to 0.24 win probability for when generating new trees. This still does not appear to have learnt to avoid trees, even though it is performing slightly better out of sample.
...
kable(prop_win_adj_newObs, caption = "Proportion of successful navigations, using previously generated trees")
...
kable(prop_winNewObjs_adj_genNewObs, caption = "Proportion of successful navigations, using newly generated trees for each loop")
```

The in sample probability has increased by 0.07 from the previous run, although still lower than from b i. However, the success probability is much higher for this run especially considering that each run uses a new generation of trees. Therefore it does appear that for this run, the model has learnt how to avoid trees in general although not perfect. 
...
resTS = game_tree(m, -1, 0, 9)
n_gTS  = 	length(resTS)
XwinsTS = sum(resTS==-1)
DrawsTS = sum(resTS== 0)
OwinsTS = sum(resTS==+1)
UnfinishedTS = sum(resTS==+2)
TS = data.frame("Games" = n_gTS,"X Wins" = XwinsTS, "Draws" = DrawsTS, "O wins" = OwinsTS, "Unfinished Games" = UnfinishedMCTS)
TSWinProb = data.frame("Prop X wins" = XwinsTS/n_gTS, "Prop Draws" = DrawsTS/n_gTS,"Prop O wins" = OwinsTS/n_gTS)

kable(MCTS, caption = "Table of results using Monte Carlo Tree Search, alpha = 0.95")
kable(TS, caption = "Table of results using Tree Search")
kable(MCTSWinProb, caption = "Table of proportions using Monte Carlo Tree Search, alpha = 0.95")
kable(TSWinProb, caption = "Table of proportions using Tree Search")
...
points(c(TS$X.Wins, TS$Prop.Draws, TS$Prop.O.wins), pch = "*", col = "red", cex = 3)
boxplot(df70[, 2:4], main = "Box plot of MCTS, alpha = 0.7")
points(c(TS$Prop.X.wins, TS$Prop.Draws, TS$Prop.O.wins), pch = "*", col = "red", cex = 3)
```
